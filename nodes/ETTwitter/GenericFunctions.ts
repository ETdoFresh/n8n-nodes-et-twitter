...
export async function twitterApiRequest(
	...
	try {
		...
		Logger.verbose(`[ETTwitter] Api Request: ${JSON.stringify(options)}`);
		//@ts-ignore
...
export async function uploadAttachments(
	...
	for (const binaryPropertyName of binaryProperties) {
		...
		if (isImage) {
			...
			Logger.debug(`[ETTwitter] Image Upload Response: ${JSON.stringify(response)}`);
			...
			Logger.debug(`[ETTwitter] Video Upload Init Response: ${JSON.stringify(response)}`);
			...
			for (const binaryPart of binaryParts) {
				...
				Logger.debug(`[ETTwitter] Video Upload Append Response: ${JSON.stringify(response)}`);
			...
			Logger.debug(`[ETTwitter] Video Upload Finalize Response: ${JSON.stringify(response)}`);

			// https://developer.twitter.com/en/docs/twitter-api/v1/media/upload-media/api-reference/get-media-upload-status
			let state = 'in_progress';
			let checkAfterSeconds = 1;
			while (state === 'in_progress') {
				await sleep(checkAfterSeconds * 1000);
				response = await twitterApiRequest.call(this, 'GET', '', {}, {
					command: 'STATUS',
					media_id: mediaId,
				}, uploadUri, {});
				
				// tslint:disable-next-line:no-any
				state = (response.processing_info as any).state;
				// tslint:disable-next-line:no-any
				checkAfterSeconds = (response.processing_info as any).check_after_secs;

				Logger.debug(`[ETTwitter] Video Upload Status Response: ${JSON.stringify(response)}`);
			}
